      ...
      <section className="help-about-dialog" role="dialog" aria-modal="true" aria-labelledby="help-about-title" tabIndex={-1}>
        ...
        <div className="help-about-content">
          ...
          <section aria-labelledby="help-about-panels">
            ...
            <p>There is no formatting toolbar: the page stays quiet. Right-click in the note for a small bar with bold, italic, code and link, which acts on whatever is selected. It is asked for rather than offered, so selecting a phrase to re-read it never puts buttons over the words. Cut, copy and paste keep their usual keys: Ctrl+X, Ctrl+C and Ctrl+V. Type <code>/</code> at the start of a line (or after a space) for a menu of things to add: headings, a quote, lists, a divider, or the picture the Images panel is showing; keep typing to narrow the list, use the arrow keys and Enter to choose, or Esc to dismiss it. Markdown shortcuts work too: <code>#</code> for a heading, <code>-</code> for a list, <code>&gt;</code> for a quote, <code>**bold**</code> and <code>*italic*</code>. To open a link in a note, hold <strong>Ctrl</strong> or <strong>Shift</strong> and select it: the pointer changes shape while you hold the key, to show which words will open. A plain click puts the cursor in the link instead, so you can change its words. The header menu offers four text sizes, from <strong>Small</strong> to <strong>Extra large</strong>; the choice is remembered.</p>
            <p>A note can also hold a live view of another panel. Drag the grip at the right of the Images panel's playback controls into a note and an embed appears there showing what that panel is showing, and following it as it changes. It starts as a poster; select <strong>Show</strong> to see it. If the panel is later removed, the embed says so. In the note's Markdown the embed is a single link line, so it travels with an exported moment.</p>
          ...
          <section aria-labelledby="help-about-moments">
            ...
            <p>The controls at the top left of the canvas manage moments. Pick one from <strong>Open moment</strong>, create one with <strong>New moment</strong>, and use <strong>Rename moment</strong>, <strong>Duplicate moment</strong>, or <strong>Delete moment</strong> on the current one; the <strong>…</strong> button beside them repeats these actions. <strong>Duplicate moment</strong> makes an independent copy, named "(copy)" or the next free number, and opens it. Deleting a moment removes this app's local copies of its images and notes after confirmation; original files and exported archives are unaffected.</p>
            <p>Use <strong>Export moment</strong> to download a portable <code>.moment.zip</code> archive, and <strong>Import moment</strong> to load one. An imported moment brings its canvas layout, settings, Markdown notes, and embedded local images; if a moment with that name already exists, you are offered a new name for the import, with a suggestion already filled in, or you can keep the original name to add it as a separate moment with the same name on purpose. Spotify login tokens and live playback data are never included.</p>
