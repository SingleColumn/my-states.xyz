  ...
  return (
    <header className="card-header" data-panel-type={panelType}>
      ...
      {/* The title is frame: pressing it drags the panel. The actions are one
          content region, so every button in it -- including the ones each
          panel passes in -- is operable without any handler of its own. */}
      <div className="card-header-actions" {...panelContentProps}>
        ...
        <div className="card-header-group">
          <button className="card-icon-button" type="button" title="Hide panel" aria-label="Hide panel" onClick={() => commands.hidePanel(panelId)}><EyeOff size={18} /></button>
          <button className="card-icon-button" type="button" title={fullScreen ? 'Restore previous panel size' : 'Expand panel to full screen'} aria-label={fullScreen ? 'Restore previous panel size' : 'Expand panel to full screen'} onClick={() => commands.togglePanelFullScreen(panelId)}>{fullScreen ? <Minimize2 size={18} /> : <Maximize2 size={18} />}</button>
          <button className="card-icon-button" type="button" title="Restore panel to default size" aria-label="Restore panel to default size" onClick={() => commands.restorePanelDefaultSize(panelId)}><RotateCcw size={18} /></button>
        </div>
